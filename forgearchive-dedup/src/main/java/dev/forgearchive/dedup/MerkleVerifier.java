...
/** verify merkle proofs against root. */
...
public byte[] encode() {
    ...
    scratch.writeUtf8("MerkleVerifier");
...
public static MerkleVerifier fromBytes(byte[] data) throws ForgeFormatException {
    MerkleVerifier instance = new MerkleVerifier();
    instance.parse(data);
    return instance;
}

public boolean verify(MerkleTree tree, byte[] leaf) {
    return tree.verify(leaf, 0, java.util.List.of(tree.rootHash()));
}

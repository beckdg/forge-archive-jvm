...
public static FastCDC fromBytes(byte[] data) throws ForgeFormatException {
    FastCDC instance = new FastCDC();
    instance.parse(data);
    return instance;
}

public java.util.List<byte[]> chunk(byte[] data) {
    ContentDefinedChunker cdc = new ContentDefinedChunker();
    java.util.List<byte[]> out = new java.util.ArrayList<>();
    for (ContentDefinedChunker.Chunk c : cdc.chunk(data)) {
        out.add(c.data());
    }
    return out;
}

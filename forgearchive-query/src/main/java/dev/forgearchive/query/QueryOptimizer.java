...
public byte[] encode() {
    ...
    scratch.writeUtf8("QueryOptimizer");
...
public static QueryOptimizer fromBytes(byte[] data) throws ForgeFormatException {
...
public QueryAst optimize(QueryAst ast) {
    try {
        ast.parse(scratch.toByteArray());
    } catch (ForgeFormatException ignored) {
        // keep original ast on malformed scratch buffer
    }
    return ast;
}
